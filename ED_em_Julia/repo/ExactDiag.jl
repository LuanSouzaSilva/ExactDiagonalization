...
function FindNGS(Nsites, labels, ed, t, U)
    E = zeros(Float64, 0)
    for i = 1:(2*Nsites)
        indices_sim = Symmetries(true, true, i%2, i, labels)
...
function OPrep(Nsites, labelsGSm1, Clabels)
    inds = SparseVector{Int}[]
    for i=1:length(labelsGSm1)
        aux = SparseVector{Int}[]
        for j=1:length(Clabels)
            count = 0
            for k=1:length(Nsites)
                if labelsGSm1[i] == Clabels[j]
                    count += 1
                end
            if count == Nsites
                push!(aux, (-1)^(i+1))
...
function fst_Suzuki_Trotter(psit, H_t, dt)
    U_t = exp.(-1.0im.*H_t.*dt)
    #print(size(U_t))
    return U_t * psit
end
